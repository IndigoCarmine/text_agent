...
class Config:
    ...
    def parse_args():
        ...
        parser.add_argument('-i','--input-file', help='入力Markdownファイル',default=r"C:\Users\taman\Dropbox\論文\xQuinNaphOMeTDP\Check\Yamada_xQuinNaphOMeTDP_ms_20260131.md")
        parser.add_argument('-o', '--output-file', help='出力ファイル名 (省略時は上書き保存)')
        parser.add_argument('-c', '--checks', nargs='+', required=True, choices=['typo', 'grammar', 'academic', 'terminology', 'logic',"logicflow"], help='有効化するチェック観点')
        parser.add_argument('--ollama-url', default='http://192.168.11.63:11434/api/generate', help='Ollama APIエンドポイント')
